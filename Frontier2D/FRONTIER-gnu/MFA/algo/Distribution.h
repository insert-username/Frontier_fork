...
public:
    ...
    static void copyLabel(Graph &g0, Graph &g1) {
        int i, nVer, nEdg;
        Vertex *v;
        Edge *e;
    ...
    static void delInnerVer(Graph &F, Graph &graph0)
    {
    ...
    static int distributEdge(Edge &edge, Graph &F, std::ostream &file2)
    {
        int a, K;
        Graph CF;
        ...
        {
            K=AlgoGraph::copyG(F, CF);   // copy F to CF and return K value
            CF.delEdg(edge);
            edge.setWeight(K);
    ...
    static int pushOutside(GlobalState& globalState,
                           Cluster &C,
                           Graph &F,
                           Graph &graph0,
                           std::ostream &file1,
                           std::ostream &file2)
    {
        ...
        for(i=1;i<=nFront;i++)    // for every frontier vertices
        {
            ...
            for(v2Name=1;v2Name<=globalState.getSingleVertex();v2Name++)
            {
        ...
        K=AlgoGraph::copyG(F, CF);    // copy from F to CF
        ...
        for(i=1;i<=nFront;i++)    // for every frontier vertices
        {
            ...
            for(v2Name=1;v2Name<globalState.getNextVerName();v2Name++)
            {
                ...
                if(targetName!=0&&v2Name!=coreName&&!frontierVs.hasElem(v2Name)
                   &&(!v2.isFrozen()))
                {
                    AlgoGraph::copyG(CF, CCF);  // copy from CF to CCF
    ...
    static int pushOutside0(Graph &F, std::ostream &file2)
    {
        ...
        AlgoGraph::copyG(F, CF);    // copy from F to CF
        ...
        for(i=1;i<=numTrivial;i++)
        {
            ...
            for(j=1;j<=nVerF;j++)
            {
                ...
                {
                    AlgoGraph::copyG(CF,CCF); // from CF to CCF
    ...
    static Cluster * distributeVert (GlobalState& globalState,
                                     Vertex &v1,
                                     Graph &F,
                                     Graph &graph0,
                                     std::ostream &file1,
            ...
            if(edgeName&&F.hasVert(v2Name))
            {
                ...
                else
                {
                    ...
                    if(k>0)
                    {
                        ...
                        {
                            if(AlgoGraph::extension(globalState, F, graph0, file1, file2)==1)
                            {
                                newTree=AlgoGraph::getCluster(globalState, F, graph0);
    ...
    // distribute Cluster
    static Cluster & distributeCl(GlobalState& globalState,
                                  Cluster &C,
                                  Graph &F,
                                  Graph &graph0,
                                  std::ostream &file1,
        ...
        if(v1Name>0&&v1Name<=globalState.getSingleVertex()) // distributing single vertex
        {
            //out<<"in distributeCl 1"<<endl;
            v1=C.returnCore();
            newTree=distributeVert(globalState, v1, F, graph0, file1, file2);
            //newTree->output(out);
        ...
        else  // distributing a cluster
        {
            ...
            k=pushOutside(globalState, C, F, graph0, file1, file2);
            if(k>0)
            {
                newTree=AlgoGraph::getCluster(globalState, F, graph0);
                return *newTree;
