...
private:
    ...
    oneNode<Type> *prev;
    oneNode<Type> *next;
    oneNode() { prev=nullptr; next=nullptr; }
    inline oneNode(const Type& elm) { elem=elm; prev=nullptr; next=nullptr; }
};
...
public:
    inline List() { length=0; first=last= nullptr; }
...
Type* List<Type>::addrByIndex(int pos)
{
    int i;
    oneNode<Type> *current;
    if(pos>length || length<=0) return nullptr;
    current=first;
    if(length>1)
        for(i=1;i<pos;i++)
            current=current->next;
...
{
    if(_position==nullptr||length<=0) return;
